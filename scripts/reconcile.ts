...
for (const account of accountRows) {
  ...
  await scriptDb.update(accounts).set({ currentBalance: account.initialBalance + Number(source.income) + Number(destination.total), updatedAt: new Date() }).where(and(eq(accounts.id, account.id), eq(accounts.userId, user.id)));
}
...
for (const wallet of walletRows) {
  const [source] = await scriptDb
    ...
    .from(transactions)
    .where(and(eq(transactions.userId, user.id), eq(transactions.walletId, wallet.id), sql`${transactions.status} in ('PAID','RECEIVED')`));
  const [destination] = await scriptDb
    .select({ total: sql<number>`coalesce(sum(${transactions.amount}), 0)` })
    .from(transactions)
    .where(and(eq(transactions.userId, user.id), eq(transactions.destinationWalletId, wallet.id), sql`${transactions.status} in ('PAID','RECEIVED')`));
  await scriptDb.update(wallets).set({ currentBalance: wallet.initialBalance + Number(source.total) + Number(destination.total), updatedAt: new Date() }).where(and(eq(wallets.id, wallet.id), eq(wallets.userId, user.id)));
}

  ...
  return (
    <>
      ...
      <FormDetails title="Cadastrar dívida">
        <MutationForm action={createDebtAction} successMessage="Dívida cadastrada." className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      {debts.length ? <div className="grid gap-4 xl:grid-cols-2">
        {debts.map((debt) => {
          ...
          return <article className="card grid gap-5 p-5 md:grid-cols-[1fr_280px]" key={debt.id}>
            ...
            <MutationForm action={updateDebtProgressAction} successMessage="Dívida atualizada." className="grid gap-2 border-t pt-4 md:col-span-2 sm:grid-cols-4"><input type="hidden" name="id" value={debt.id} /><Field label="Saldo atual"><input className="field" name="currentBalance" defaultValue={(debt.currentBalance / 100).toFixed(2)} required /></Field><Field label="Parcelas pagas"><input className="field" name="paidInstallments" type="number" min="0" defaultValue={debt.paidInstallments} required /></Field><Field label="Situação"><select className="field" name="status" defaultValue={debt.status} required><option value="ACTIVE">Ativa</option><option value="OVERDUE">Atrasada</option><option value="PAID">Quitada</option><option value="RENEGOTIATED">Renegociada</option></select></Field><div className="flex items-end"><button className="btn btn-secondary">Atualizar</button></div></MutationForm>
          </article>;
        })}

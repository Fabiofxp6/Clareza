    ...
    <>
      ...
      <section className="card">
        {rows.length ? <div className="table-wrap"><table className="data-table"><thead><tr><th>Conta</th><th>Categoria</th><th>Valor médio</th><th>Vencimento</th><th>Pagamento</th><th>Gerar lançamento</th></tr></thead><tbody>
          {rows.map(({ item, category, account }) => <tr key={item.id}><td><div className="font-semibold">{item.name}</div><div className="text-xs text-[var(--muted)]">{item.automaticDebit ? "Débito automático" : "Pagamento manual"}</div></td><td>{category}</td><td>{formatCurrency(item.averageAmount)}</td><td>Dia {item.dueDay}</td><td>{account ?? "—"}</td><td><div className="flex gap-2"><form action={generateFixedExpenseAction} className="flex gap-2"><input type="hidden" name="id" value={item.id} /><select className="field !min-h-8 !py-1" name="month" defaultValue={month}>{Array.from({ length: 12 }, (_, i) => <option value={i + 1} key={i}>{monthLabel(i + 1)}</option>)}</select><input className="field w-20 !min-h-8 !py-1" name="year" type="number" value={year} readOnly /><button className="btn btn-secondary !min-h-8 !px-2 text-xs">Gerar</button></form><form action={toggleActiveAction}><input type="hidden" name="id" value={item.id} /><input type="hidden" name="entity" value="fixed" /><input type="hidden" name="active" value={String(!item.isActive)} /><button className="btn btn-secondary !min-h-8 !px-2 text-xs">{item.isActive ? "Desativar" : "Reativar"}</button></form></div></td></tr>)}
